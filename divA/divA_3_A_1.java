...
public class Test {
    ...
    public static void main(String args[]) {
        ...
        Set<Integer> possibleNumbers = IntStream.rangeClosed(1, num).boxed().collect(Collectors.toCollection(HashSet::new));
        StringBuilder answer = new StringBuilder();
        String input = scanner.nextLine();
        while (!input.equals("HELP")) {
            Set<Integer> question = Arrays.stream(input.split(" "))
                    .map(Integer::parseInt)
                    .filter(possibleNumbers::contains)
                    .collect(Collectors.toCollection(HashSet::new));
            if (question.size() <= possibleNumbers.size()/2) {
                answer.append("NO\n");
                possibleNumbers.removeAll(question);
            } else {
                answer.append("YES\n");
                possibleNumbers = question;
            }
            input = scanner.nextLine();
        }

        System.out.print(answer);
        System.out.println(possibleNumbers.stream().map(Object::toString).collect(Collectors.joining(" ")));
    }
}