 ...
 * Первая строка содержит число 0. Вторая строка содержит название темы. Длина названия 
 * не превышает 30 символов. Третья строка содержит текст сообщения.
 * 
 ...
 * 
 * // Формат вывода
 * Выведите название темы, к которой относится наибольшее количество сообщений. Если таких
 * тем несколько, то выведите первую в хронологическом порядке 
 * 
 * // Примеры
 * 7
 * 0
...
public class Test {
    ...
    public static void main(String args[]) {
        ...
        Map<Integer, Topic> codes = new HashMap<>();
        for (int i = 1; i <= numOfMessages; i++) {
            String topicCode = scanner.nextLine();
            if (topicCode.equals("0")) {
                codes.put(i, new Topic(scanner.nextLine(), 1));
            } else {
                Topic topic = codes.get(Integer.parseInt(topicCode));
                codes.put(i, topic);
                topic.inc();
            }
            scanner.nextLine();
        ...
        codes.values().stream()
                .distinct()
                .sorted((topic1, topic2) -> topic2.posts.compareTo(topic1.posts))
                .limit(1)
                .forEach(topic -> System.out.println(topic.name));
    }
